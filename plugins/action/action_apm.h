...
class APMActionImpl : public ActionImpl
{
public:
    APMActionImpl();
    ~APMActionImpl();
...
private:
    void guided_before_takeoff_async(const Action::result_callback_t &callback);
    void guided_before_arm_async(const Action::result_callback_t &callback);
    ...
    void process_sys_state(const mavlink_message_t &message);
